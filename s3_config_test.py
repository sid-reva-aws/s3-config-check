...
# The original python script idea was inspired by RhinoSecurity blog and developed/modified for use in test case scenario
import argparse
import sys
...
def main(args):
    if args.profile is None:
        ...
        print('No AWS CLI profile passed in, Please rerun the script using the -p or --profile argument:')
        ...
        try:
            ...
            if versioning == 'Enabled' and mfa_delete == 'Enabled':
                csv_rows.append([
                    ...
                    versioning,
                    mfa_delete,
                    'Bucket has secured configuration enabled',
                    'None'
                ])
                ...
                csv_rows.append([
                    ...
                    versioning,
                    mfa_delete,
                    '"Bucket has versioning enabled, but not MFA Delete"',
                    'Enable MFA delete'
                ])
            else:
                csv_rows.append([
                    ...
                    versioning,
                    mfa_delete,
                    'Bucket can be VULNERABLE to malicious attacks',
    ...
    csv_file_name = f'{profile_name}_S3_bucket_config_scan_results_{str(time()).split(".")[0]}.csv'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--profile', required=False, default=None, help='The AWS CLI profile to use for making API calls. This is usually stored under ~/.aws/credentials')
